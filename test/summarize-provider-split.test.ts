...
vi.mock("../src/providers/anthropic.js", () => ({
  AnthropicProvider: class {
    name = "anthropic";
    constructor(_key: string, private model: string, _max: number, private baseURL?: string) {}
    async compress() { calls.push(`compress:anthropic:${this.model}`); return ""; }
    async summarize() { calls.push(`summarize:anthropic:${this.model}@${this.baseURL}`); return ""; }
  },
}));
...
describe("summarize provider split (#899)", () => {
  ...
  async function build(fallback: string[] = []) {
    ...
    return fallback.length
      ? createFallbackProvider(primary, { providers: fallback as never })
  ...
  it("routes summarize() to the configured provider and compress() to the primary", async () => {
    ...
    expect(calls).toEqual([
      ...
      "summarize:anthropic:claude-opus-5@http://proxy.local",
    ]);
  });
  ...
  it("keeps the split on the primary inside a fallback chain", async () => {
    process.env.AGENTMEMORY_SUMMARIZE_PROVIDER = "anthropic";
    process.env.ANTHROPIC_MODEL = "claude-opus-5";
    ...
    expect(calls).toEqual([
      "compress:openai:glm-5.3-flash",
      "summarize:anthropic:claude-opus-5@undefined",
    ]);
  });
});
