...
- **Telemetry-only hooks** (`notification`, `post-tool-failure`, `post-tool-use`, `prompt-submit`, `stop`, `session-end`, `subagent-start`, `subagent-stop`, `task-completed`) write nothing to stdout. These MUST use fire-and-forget `fetch(..., { signal: AbortSignal.timeout(N) }).catch(() => {})` paired with an unref'd `setTimeout(() => process.exit(0), timeout)`. The unawaited fetch dispatches the request; the timer force-exits the process after the request has had time to complete. Use 1500ms for `prompt-submit` because a remote `AGENTMEMORY_URL` can take more than 500ms to acknowledge the request, and for multi-request hooks like `stop` and `session-end` so all fetches have time to start. Use 500ms for other single-request hooks. Without the `setTimeout` Node keeps the event loop alive waiting for any in-flight fetch to settle, which means the hook still blocks Claude Code's next-prompt boundary for up to the AbortSignal duration — exactly the bug fire-and-forget is meant to fix.
