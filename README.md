...
Codex's hook engine injects `CLAUDE_PLUGIN_ROOT` into hook subprocesses (per [`codex-rs/hooks/src/engine/discovery.rs`](https://github.com/openai/codex/blob/main/codex-rs/hooks/src/engine/discovery.rs)), so the same hook scripts work across both hosts without duplication. Subagent / SessionEnd / Notification / TaskCompleted / PostToolUseFailure events are Claude-Code-only and are not registered for Codex. This fork additionally leaves `PreToolUse` and `PreCompact` out of `plugin/hooks/hooks.codex.json` until their Codex-specific input and output contracts are supported end to end; `test/codex-plugin.test.ts` asserts the trim, and the Windows Codex global fallback makes the same cut for the same reason (`plugin/skills/agentmemory-hooks/REFERENCE.md`).
...
| **Codex CLI (full plugin)** | Codex plugin marketplace | `codex plugin marketplace add rohitg00/agentmemory` then `codex plugin add agentmemory@agentmemory`. Registers MCP + 4 lifecycle hooks (SessionStart, UserPromptSubmit, PostToolUse, Stop) + 17 skills. On Codex Desktop, also run `agentmemory connect codex --with-hooks` until [openai/codex#16430](https://github.com/openai/codex/issues/16430) lands; plugin hooks are currently silent there. |
...
The server then exposes `POST /mcp` on the same loopback REST port (`GET`/`DELETE /mcp` answer 405: the endpoint is stateless and has no SSE stream). The endpoint supports both the legacy initialize lifecycle and the stateless MCP `2026-07-28` discovery lifecycle. Result sizes are bounded at the MCP boundary (`memory_recall` and `memory_lesson_recall` limit 1–100, `memory_sessions` limit 1–100 with a `total` field, `memory_timeline` before/after ≤ 50). It intentionally exposes only eight read-only tools: `memory_recall`, `memory_file_history`, `memory_sessions`, `memory_smart_search`, `memory_timeline`, `memory_commit_lookup`, `memory_graph_query`, and `memory_lesson_recall`.
...
This opt-in MVP recalls stored lessons by relevance and uses one adoption catalog for explicitly confirmed knowledge. Each namespace permits 12 catalog records (including candidates and superseded rules) and 256 change events. An explicit save matching an existing lesson in the same project links that lesson by ID; it does not create a second editable lesson. The warm BM25 index takes at most ten hits from the whole lesson corpus before project comparison. Ungoverned lessons, including lessons from other projects and automatically extracted lessons, can be judged and injected without user adoption; catalog-governed lessons keep their project scope. The later same-project sort does not guarantee priority because the search and final candidate budget can already exclude them. Jev decides relevance for the remaining candidates. A separate ten-candidate and 6,000-character limit applies before the judge request. No vector search is required. The response reports `candidates: { catalog, lessons }` before the final budget, so these counts are not the number Jev received.

The prompt client independently rejects malformed, duplicate, or oversized selections (more than two spans or 1,200 text characters), rather than injecting a partial response. Historical lesson saves deduplicate within the supplied project scope; matching legacy IDs are preserved only within that same scope. This does not establish repository identity or adoption evidence for historical lessons; they are recalled by relevance, not by adoption.
...
The shared `agent-hooks` integration uses that same private configuration with `owner: "agent-hooks"`, an absolute `clientScript` pointing to the installed `plugin/scripts/selective-client.mjs`, and an optional absolute `node` runtime path. Shared ownership suppresses standalone prompt-hook recall, not ordinary telemetry. The shared client performs selection without recording ordinary prompts a second time; explicit memory requests retain their source-backed capture flow. Native adapters pass normalized JSON through `selective-client.mjs --stdin` and render the resulting bounded text. This is an opt-in adapter contract, not a claim of fleet deployment. Historical lessons are automatic candidates; adoption evidence only adds catalog scope and replacement semantics.

Shared recall appends `kind: "memory"` events to the existing `~/.local/state/agent-hooks/injections.jsonl`, with session, harness, machine, elapsed time, output characters and a bounded reason code. Client/API timeouts, HTTP errors, invalid responses, disabled configuration and API empty selections are distinct. These records measure shared-client output, not platform delivery or model adherence. With a current server, the `candidates` field reports the pool before final eligibility and budget checks; older servers omit it. An `api_empty` result with zero candidates means none were found; a positive count means none were selected, whether filtered before judgment or rejected by Jev. No prompt, knowledge text, credentials or raw error bodies are logged. Inspect with `jq 'select(.memory)' ~/.local/state/agent-hooks/injections.jsonl`.

Run `npm run build` and `npm run test:mvp` for the bounded, offline acceptance suite. It covers source-backed capture and replacement, scope isolation, invalid/expired decisions, dialogue extraction, backup preservation, and the packaged hook's output and timeout behavior. These tests do not call paid APIs or prove model adherence. The opt-in live Jev evaluation in `test/selective-context-live.test.ts` checks semantic decisions separately; actual harness acceptance requires sampling selected text for usefulness and checking that the model uses relevant facts without repeating irrelevant ones. Selection counts alone do not measure precision. Passing this suite is not equivalent to passing the repository-wide suite or deploying the feature.
