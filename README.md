...
This opt-in MVP recalls explicitly confirmed knowledge using one adoption catalog. Each namespace permits 12 records (including candidates and superseded rules) and 256 change events. An explicit save matching an existing lesson in the same project links that lesson by ID; it does not create a second editable lesson. Candidates come from the whole lesson corpus through the existing warm BM25 index (at most ten hits): every stored lesson is a candidate regardless of project, same-project hits are ordered first, and the Jev decision decides relevance. Lessons the catalog governs keep the catalog's project scope instead. Status, source evidence, ten-candidate and 6,000-character limits apply before one Jev request. No vector search is required. The response reports `candidates: { catalog, lessons }` so an empty result can be attributed to no candidates or to the judge.
