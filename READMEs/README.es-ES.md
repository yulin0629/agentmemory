...
- 4 hooks de ciclo de vida: `SessionStart`, `UserPromptSubmit`, `PostToolUse`, `Stop`
...
| **Codex CLI (plugin completo)** | Marketplace de plugins Codex | `codex plugin marketplace add rohitg00/agentmemory` y luego `codex plugin add agentmemory@agentmemory`. Registra MCP + 4 hooks de ciclo de vida (SessionStart, UserPromptSubmit, PostToolUse, Stop) + 17 skills. En Codex Desktop, ejecuta también `agentmemory connect codex --with-hooks` hasta que aterrice [openai/codex#16430](https://github.com/openai/codex/issues/16430); los hooks de plugin están silenciados allí por ahora. |
