...
- 4 yaşam döngüsü hook'u: `SessionStart`, `UserPromptSubmit`, `PostToolUse`, `Stop`
...
| **Codex CLI (tam eklenti)** | Codex eklenti marketplace | `codex plugin marketplace add rohitg00/agentmemory` ardından `codex plugin add agentmemory@agentmemory`. MCP + 4 yaşam döngüsü hook'u (SessionStart, UserPromptSubmit, PostToolUse, Stop) + 17 skill kaydeder. Codex Desktop'ta, [openai/codex#16430](https://github.com/openai/codex/issues/16430) inene kadar `agentmemory connect codex --with-hooks` da çalıştırın; eklenti hook'ları şu anda orada sessiz. |
